...
foreach ($mintName in @('cost-basis.mjs','server.mjs','engine.mjs','coordinator.mjs','lib.mjs','nft-market.mjs','monitor-service.mjs','opensea-page.mjs','project-market.mjs','run-lock.mjs','package.json','package-lock.json','README.md','LICENSE','THIRD-PARTY-NOTICES.md','START.cmd','Mint Desk.exe','Mint Desk.ico')) {
 $mintEntries.Add(@{File=(Get-Item -LiteralPath (Join-Path $mintRoot $mintName)).FullName;Relative=$mintName})
}
