...
function monitorCard(r){const card=el('article',undefined,'project-card');card.append(nftArtwork.heading([el('span',r.earlyLabel||(r.source==='chain'?`原始发现 · ${r.status||'待核验'}`:r.status||'待核验'),r.source==='chain'?'badge raw-badge':'badge'),monitorName(r)],{chainId:r.chain==='ethereum'?1:4663,contract:r.address,name:r.name||r.slug}),el('div',r.address||'合约尚未识别','address'));const meta=el('div',undefined,'meta');
 ...
 if(r.timeDisplayWarning)fields.push(['时间提示',r.timeDisplayWarning]);
...
function drawMonitor(){for(const card of document.querySelectorAll('[data-monitor-category]'))card.setAttribute('aria-pressed',String(card.dataset.monitorCategory===$('monitor-filter').value));const s=monitorState;if(!s)return;
 ...
 const feed=$('monitor-events');feed.replaceChildren();for(const a of [...alerts].slice(-6).reverse()){const r=priority.find(r=>r.address.toLowerCase()===a.address?.toLowerCase());feed.append(nftArtwork.heading([el('p',`${localTime(a.time)} · ${r?.name||a.address} · ${a.message}`)],{chainId:4663,contract:a.address,name:r?.name}));}
 for(const r of priority.slice(0,6))feed.append(nftArtwork.heading([el('p',`${r.name||r.slug} · 估算净利润 / 成本 ${Number(r.profitToCost).toFixed(2)} 倍 · 行情 ${localTime(r.marketCheckedUtc)} · 仍需核实可成交性`)],{chainId:4663,contract:r.address,name:r.name}));
