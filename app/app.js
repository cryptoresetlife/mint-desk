...
function show(name){page=name;for(const n of document.querySelectorAll('.page'))n.hidden=n.id!==`page-${name}`;for(const n of document.querySelectorAll('.nav'))n.classList.toggle('active',n.dataset.page===name);$('page-title').textContent={tracker:'重点钱包',monitor:'机会监控',projects:'我的项目',wallets:'钱包管理',nfts:'我的 NFT',tasks:'运行任务',history:'交易记录'}[name];$('page-subtitle').textContent={tracker:'追踪公开钱包的链上行动，发现多人参与，再核对项目。',monitor:'发现项目、复核成交证据，再选择是否加入 mint。',projects:'添加目标，设置预算，让准备工作提前完成。',wallets:'只在本机输入，每次启动重新导入。',nfts:'查看持仓、核对行情，自行定价后批量上架。',tasks:'等待、预演、提交与回执，进展一目了然。',history:'保留交易证据，避免不确定时重复发送。'}[name];if(name==='tracker')window.mintTracker?.activate();if(name==='nfts')window.mintHoldings?.activate();if(name==='history')loadHistory().catch(e=>note(e.message,true));}
...
function draw(){
  ...
  for(const p of state.projects){
    const card=el('article',undefined,'project-card');card.append(nftArtwork.heading([el('span',`${state.chains[p.chainId]?.name??p.chainId} · Public`,'badge'),el('h3',p.name)],{chainId:p.chainId,contract:p.address,name:p.name}),el('div',p.address,'address'));
