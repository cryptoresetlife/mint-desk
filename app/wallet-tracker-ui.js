(()=>{
 ...
 function render(){if(!snapshot)return;const s=snapshot;
  ...
  for(const a of s.alerts){const row=el('div',undefined,'panel');row.append(el('strong',a.message),el('p',`${short(a.contract)} · ${stamp(a.at)} · 收益未知`));row.append(button('查看相关活动','quiet',()=>{selectedContract=a.contract;$('tracker-filter').value='all';signature='';render();$('tracker-events').scrollIntoView({behavior:'smooth'});}));alerts.append(row);if(sound&&!seen.has(a.id)){const osc=audio.createOscillator(),gain=audio.createGain();gain.gain.value=0.06;osc.connect(gain).connect(audio.destination);osc.frequency.value=a.kind==='sell'?440:740;osc.start();osc.stop(audio.currentTime+0.15);}seen.add(a.id);}
  ...
  for(const e of list){const card=el('article',undefined,'project-card');card.append(el('span',kinds[e.kind]||e.kind,'badge'),externalLink(`${e.name||short(e.contract)} #${e.tokenId}`,`https://opensea.io/assets/robinhood/${e.contract}/${e.tokenId}`),el('p',`${e.label} · ${e.group}${e.focus?' · 重点':''}`),externalLink(short(e.wallet),`https://robinhoodchain.blockscout.com/address/${e.wallet}`),el('p',`${stamp(e.at)} · 数量 ${e.quantity}${e.historical?' · 历史补查':''}`));
